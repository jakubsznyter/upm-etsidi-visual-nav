...
class TrajectoryMaster(Node):
    def __init__(self):
        ...
        # Figure-eight waypoints (Double Box)
        self.waypoints = [
            (1.0, 0.0), (1.0, 1.0), (0.0, 1.0), # Upper square
            (0.0, -1.0),                        # Long transition
            (1.0, -1.0), (1.0, 0.0), (0.0, 0.0)  # Lower square + Start
        ]
    ...
    def navigate(self):
        ...
        # Rotate-then-drive logic
        if abs(angle_diff) > 0.15:
            msg.angular.z = 0.4 if angle_diff > 0 else -0.4
        else:
            msg.linear.x = 0.25
            msg.angular.z = 0.5 * angle_diff # P-gain correction
